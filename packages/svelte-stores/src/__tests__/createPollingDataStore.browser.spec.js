...
/** @typedef {[a: number, b: string, c: Date]} TestArgs */
/** @typedef {{ a: number }} T */
...
describe("createPollingDataStore (browser)", () => {
  ...
  /** @type {import("../..").PollingDataStore<TestArgs, T>} */
  ...
  it("should start polling data with the desired interval when the `start` method is called and stop when the `stop` method is called", async () => {
    ...
    pollingDataStore.start(...args1);

    expect(dataRetriever).toHaveBeenCalledTimes(1);
    expect(dataRetriever).toHaveBeenCalledWith(...args1);
    expect(get(pollingDataStore)).toStrictEqual({
      data: null,
      error: null,
      isLoading: true,
    ...
    expect(dataRetriever).toHaveBeenNthCalledWith(2, ...args1);
    expect(get(pollingDataStore)).toStrictEqual({
      data: data1,
      error: null,
      isLoading: true,
    ...
    expect(dataRetriever).toHaveBeenNthCalledWith(3, ...args1);
    expect(get(pollingDataStore)).toStrictEqual({
      data: data2,
      error: null,
      isLoading: true,
  ...
  it("should start a new poll process and stop the previous one when the `start` method is called and a polling is running", async () => {
    ...
    expect(dataRetriever).toHaveBeenCalledTimes(1);
    expect(dataRetriever).toHaveBeenNthCalledWith(1, ...args1);
    expect(get(pollingDataStore)).toStrictEqual({
      data: null,
      error: null,
      isLoading: true,
    ...
    pollingDataStore.start(...args2);

    expect(dataRetriever).toHaveBeenCalledTimes(2);
    expect(dataRetriever).toHaveBeenNthCalledWith(2, ...args2);
    ...
    for (let i = 3; i <= 12; i++) {
      expect(dataRetriever).toHaveBeenNthCalledWith(i, ...args2);
    }
